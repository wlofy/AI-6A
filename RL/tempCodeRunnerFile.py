...
def monte_carlo_control():
    ...
    returns_count = np.zeros((n_states, n_actions))#counting the number of times a state has been visited
    ...
    start_time = time.time() #tracking the time for it to train
    for episode in tqdm(range(N_EPISODES), desc='Monte Carlo Control'):#making a progress bar to show the training progress ):
        action = epsilon_greedy_policy(Q, state)  # Select action using epsilon-greedy policy
...
def animate_policy(policy, filename, num_of_episodes = 10):
    frames = []
